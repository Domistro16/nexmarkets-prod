...
test('Postgres chain worker confirms/finalizes and records idempotent evidence', { skip: !process.env.DATABASE_URL }, async () => {
  ...
  try {
    ...
    const result = await worker.runOnce(); assert.equal(result.progressed, 3);
    const state = await pool.query('SELECT state,confirmations FROM chain_transaction WHERE id=$1', [txId]); assert.equal(state.rows[0].state, 'FINALIZED'); assert.equal(Number(state.rows[0].confirmations), 3);
