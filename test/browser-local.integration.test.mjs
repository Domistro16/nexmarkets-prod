...
test('local browser workflow has no Builder deployment request and tracks holder transactions', async (t) => {
  ...
  await fetch(`${base}/v1/builder/projects`, { ...headers, method: 'POST', headers: { ...headers, 'idempotency-key': 'browser-project' }, body: JSON.stringify({ slug: `browser-${Date.now()}`, name: 'Browser flow' }) });
  const removed = await fetch(`${base}/v1/editions/prepare`, { ...headers, method: 'POST', headers: { ...headers, 'idempotency-key': 'browser-edition' }, body: '{}' });
