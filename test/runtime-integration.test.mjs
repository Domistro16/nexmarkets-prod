...
test('Postgres Goldsky raw log projects Edition and advances checkpoint', { skip: !process.env.DATABASE_URL }, async (t) => {
  ...
  const chainId = 46630;
  const accountId = `acct_it_${suffix}`; const projectId = `prj_it_${suffix}`;
  ...
  try {
    ...
    const factory = new Interface(['event EditionCreated(address indexed edition,bytes32 indexed editionId,address indexed publisher,bytes32 salt,address editionOwner,address mintController,uint32 absoluteSupplyCap,bytes32 artworkCommitment)']);
    ...
    assert.equal(editionRow.rows[0].project_id, null); assert.equal(editionRow.rows[0].absolute_supply_cap, 10);
