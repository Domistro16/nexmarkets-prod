...
test('Postgres projector routes context-free events and restores canonical state after reorgs', { skip: !process.env.DATABASE_URL }, async () => {
  ...
  try {
    ...
    const worker = new PostgresProjectionWorker({ pool, chainId, rpc: { async getBlockNumber() { return 100; }, async getBlockByNumber(number) { return { hash: hash(String(number)) }; } }, finalityDepth: 2, batchSize: 100 });
    ...
    await worker.runOnce();
    assert.equal((await pool.query('SELECT owner_address FROM pass_token_projection WHERE edition_id=$1 AND token_id=1', [editionId])).rows[0].owner_address, addr(13));
    await pool.query('UPDATE goldsky_raw_log SET removed=true WHERE chain_id=$1 AND transaction_hash=$2', [chainId, hash('1')]);
    await worker.runOnce();
    ...
    await pool.query('DELETE FROM seaport_fulfillment_projection WHERE chain_id=$1', [chainId]); await pool.query('DELETE FROM listing_event WHERE chain_id=$1', [chainId]); await pool.query('DELETE FROM royalty_claim_projection WHERE edition_id=$1', [editionId]); await pool.query('DELETE FROM listing_projection WHERE edition_id=$1', [editionId]); await pool.query('DELETE FROM advantage_state_projection WHERE edition_id=$1', [editionId]); await pool.query('DELETE FROM advantage_definition WHERE edition_id=$1', [editionId]); await pool.query('DELETE FROM pass_token_projection WHERE edition_id=$1', [editionId]); await pool.query('DELETE FROM terms_version WHERE edition_id=$1', [editionId]); await pool.query('DELETE FROM notification WHERE business_key LIKE $1', [`${chainId}:%`]); await pool.query('DELETE FROM outbox_event WHERE business_key LIKE $1', [`${chainId}:%`]); await pool.query('DELETE FROM edition WHERE id=$1', [editionId]); await pool.query('DELETE FROM indexer_event WHERE chain_id=$1', [chainId]); await pool.query('DELETE FROM goldsky_raw_log WHERE chain_id=$1', [chainId]); await pool.query('DELETE FROM indexer_checkpoint WHERE chain_id=$1', [chainId]); await pool.query('DELETE FROM terms_advantage_commitment WHERE advantages_hash=$1', [advantagesHash]); await pool.query('DELETE FROM project WHERE id=$1', [project]); await pool.query('DELETE FROM account WHERE id=$1', [account]); await pool.end();
  }
});
