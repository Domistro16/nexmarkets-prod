...
export function validateDeploymentManifest(m, { strict = false } = {}) {
  ...
  if (m.chainId === 8453 || m.chainId === 84532) {
    const usdc = m.primitives?.usdc;
    const expected = m.chainId === 8453 ? '0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913' : '0x036CbD53842c5426634e7929541eC2318f3dCF7e';
    assert(usdc && ADDRESS.test(usdc.address || ''), 'Base USDC address required');
    assert(usdc.address.toLowerCase() === expected.toLowerCase(), 'wrong canonical Base USDC address');
    assert(usdc.expectedSymbol === 'USDC', 'Base settlement must be USDC');
    assert(usdc.expectedDecimals === 6, 'Base USDC must use 6 decimals');
  }
  ...
  if (strict && (m.chainId === 8453 || m.chainId === 84532)) {
    assert(HEX_32.test(m.primitives.usdc.expectedRuntimeCodeHash || ''), 'Base USDC runtime code hash must be pinned');
  }
