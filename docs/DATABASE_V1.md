...
Migrations live in `infra/schema` and are checksum-locked after application. `0002_nexmarkets_v1.sql` adds accounts/wallets/sessions, projects, Editions, Terms versions, exact Pass projections, serial artwork, Advantages, transaction jobs, listings, seller-signed Seaport fulfillment payloads, royalty claims, referral ledgers, notifications/outbox, audit logs, Goldsky ingestion/checkpoints, reconciliation and media metadata.
