...
Migrations live in `infra/schema` and are checksum-locked after application. `0002_nexmarkets_v1.sql` adds accounts/wallets/sessions, projects, Editions, Terms versions, exact Pass projections, serial artwork, Advantages, transaction jobs, listings, seller-signed Seaport fulfillment payloads and settlement evidence, royalty claims, referral ledgers, notifications/outbox, audit logs, Goldsky raw logs plus landed block watermarks/checkpoints, reconciliation and media metadata. Advantage projections retain per-Advantage listing timestamps and frozen seconds so TimeBased/Connected read values match the registry semantics.
