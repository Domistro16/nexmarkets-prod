...
Goldsky Subgraph is a read model, never authority. `ReconciliationService` compares independent Edition, Pass, Advantage, Listing, RoyaltyVault and TBA entities from the Subgraph with RPC chain reads for Edition existence, total minted, `ownerOf`, token Terms, active Terms, Advantage state, listing/direct-transfer status, Vault obligations/withdrawals and ERC-6551 identity. It never forms a SQL Pass×Advantage×Listing×Royalty cross-product; every order hash is reconciled as its own listing/claim item. A deterministic but not-yet-created TBA is not reported as a discrepancy when the Subgraph has no created account.
...
monitor Goldsky Subgraph `_meta` freshness against the Robinhood RPC head,
reconciliation lag, incident count and retry exhaustion; the latest protocol
event is not an indexing watermark.
