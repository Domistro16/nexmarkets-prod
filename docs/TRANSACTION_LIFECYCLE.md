...
Each mutation uses an account-scoped idempotency key. Mint intent, Terms publish, listing create/cancel, royalty withdrawal, referral settlement and outbox delivery have unique constraints. Transaction jobs use capped exponential polling/backoff and record the last error without abandoning a still-pending legitimate transaction; notifications cannot change business state.
# Runtime workers
...
notifications. A hash alone is never success. It verifies the expected
target and calldata/input as well as receipt and block evidence. Capped
backoff leaves pending jobs monitored rather than permanently abandoning
them after a retry count.
